...
for (ii in 1:length(fechas_val)) {
  ...
  # load(paste("mcmc_defunciones/", maxfecha, "-modeltest.RData", sep=""))
  ...
  # 
  # ps_mod3 <- modelo_test$BUGSoutput$mean$p
  # NN_mod3 <- modelo_test$BUGSoutput$mean$NN
  # NN_q25_mod3 <- modelo_test$BUGSoutput$summary[paste("NN[", 1:length(NN_mod2), "]", sep=""),"2.5%"]
  # NN_q975_mod3 <- modelo_test$BUGSoutput$summary[paste("NN[", 1:length(NN_mod2), "]", sep=""),"97.5%"]



  as.data.frame(modelo$BUGSoutput$sims.list$p[sample(3000, 1000),]) %>%
    mutate(sim=1:1000) %>%
